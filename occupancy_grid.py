...
def generate_obstacles(screen):
    pygame.draw.rect(screen,(0,0,0),pygame.Rect(300,140,160,140))
    pygame.draw.rect(screen,(255,0,0),pygame.Rect(20,400,120,60))
    pygame.draw.rect(screen,(255,0,0),pygame.Rect(400,400,120,60))

def compute_obsmap(field):
    blockSize = 20
    obs1x,obs1y = 300//blockSize,140//blockSize
    ...
    for i in range(obs1x,obs1x+(160//blockSize)+1):
        for j in range(obs1y,obs1y+(140//blockSize)+1):
            field[i][j] = 1
    for i in range(obs2x,obs2x+(120//blockSize)+1):
        for j in range(obs2y,obs2y+(60//blockSize)+1):
            field[i][j] = 1
    for i in range(obs3x,obs3x+(120//blockSize)+1):
    ...
    def navigate(self,path):
        if self.i<len(path):
            ...
            if ind == len(path)-1:
                ...
                ang = math.degrees(math.atan2((path[ind][1]-path[ind-1][1]),(path[ind][0]-path[ind-1][0])))
            self.angle = ang
            self.new_image = pygame.transform.rotate(self.surf, -self.angle)
            self.rect = self.new_image.get_rect()
            pygame.draw.circle(self.new_image, RED, ((self.rect.centerx-(self.rect.width/4)),self.rect.centery-(self.rect.width/4)),4)
            self.rect.centerx = (path[ind][0]*20)
            self.rect.centery = (path[ind][1]*20)
            # ((self.rect.centerx-(self.rect.width*math.cos(ang)/4)),self.rect.centery-(self.rect.width*math.sin(ang)/4))
